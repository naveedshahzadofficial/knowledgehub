            ...
            <div class="card card-custom ">
                <div class="card-header flex-wrap py-3">
                    <div class="card-title">
                        <h3 class="card-label">Update Staff</h3>
                    </div>
                </div>

                <div class="card-body p-0">
                    {{ Form::open(array('route' => ['admin.admins.update',$admin],'method'=>'PUT','class'=>'form form-horizontal','name'=>'admin_form','id'=>'admin_form')) }}
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input maxlength="255" type="text" class="form-control  @error('cnic_no') is-invalid @enderror" name="cnic_no"
                                           value="{{ old('cnic_no', $admin->cnic_no) }}"
                                           id="cnic_no"  />
                                    @error('cnic_no')
                                    <div class="invalid-feedback d-block">
                                        {{ $message }}
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="role_ids[]" class="form-control select2  @error('role_ids') is-invalid @enderror" required multiple>
                                        @foreach($roles as $role)
                                            <option value="{{ $role->id }}"  {{ in_array($role->id, old('role_ids', $admin->roles->pluck('id')->toArray()) ?: []) ? 'selected' : '' }}>{{ $role->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('role_ids')
                                    <div class="invalid-feedback d-block">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="department_id" class="form-control select2  @error('department_id') is-invalid @enderror">
                                        ...
                                        @foreach($departments as $department)
                                            <option value="{{ $department->id }}" {{ old('department_id', $admin->department_id)===$department->id?'selected':'' }}>{{ $department->department_name }}</option>
                                        @endforeach
                                    </select>
                                    @error('department_id')
                                    <div class="invalid-feedback d-block">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="radio-inline">
                                        <label class="radio radio-success">
                                            <input type="radio"  @if(old('admin_status', $admin->admin_status)=='1')checked="checked"@endif name="admin_status" value="1">
                                            <span></span>Active</label>

                                        <label class="radio radio-danger">
                                            <input type="radio"  @if(old('admin_status', $admin->admin_status)=='0')checked="checked"@endif name="admin_status" value="0">
